...
class AnagramsSuite {
  ...
  @Test def `subtract: larry - r (10pts)`: Unit = {
    val larry = List(('a', 1), ('l', 1), ('r', 2), ('y', 1))
    val r = List(('r', 1))
    val lary = List(('a', 1), ('l', 1), ('r', 1), ('y', 1))
    assertEquals(lary, subtract(larry, r))
  }
  ...
  @Test def `combinations: aa `: Unit = {
    ...
    assertEquals(aaComb.toSet, combinations(aa).toSet)
  }
  ...
  @Test def `sentence anagrams: Linx (10pts)`: Unit = {
    val sentence = List("Linx")
